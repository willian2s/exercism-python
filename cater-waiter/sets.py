...
def check_drinks(drink_name, drink_ingredients):
    ...
    """
    alcohol = set(ALCOHOLS)

    has_alcohol = bool(set(drink_ingredients
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    return f"{dish_name}: {category}" if category is not None else f"{dish_name}: Category not found"
...
def separate_appetizers(dishes, appetizers):
    ...
    result = unique_dishes.difference(unique_appetizers)
...
def singleton_ingredients(dishes, intersection):
    ...
    all_category_ingredients = set().union(*dishes)

    singleton = all_category_ingredients.difference(intersection)
